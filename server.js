...
app.post('/hyundai/login', function (req, res) {
	...
	if (
		...
		con.query('SELECT service.user.id AS `id`,service.user.user_name AS `user_name`,service.user.user_id AS `user_id`,service.user.name AS `name`,service.user.phone AS `phone`,service.user.email AS `email`,service.role.id AS `role_id`,service.role.name AS `role_name`FROM user JOIN service.role on service.role.id = service.user.role_id where user_id = ? and password = ?', [userId, password], function (err, rows, fields) {
			if (!err) {
				var response = [];
