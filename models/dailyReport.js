...
var DailyReport = {
    ...
    getDailyReportById: function (id,date, callback) {
    ...
    updateReport: function (report, callback) {
        var query = 'UPDATE daily_report SET created_date = ?, line_id = ?, equipment_id = ?, break_down_start = ?, break_down_finish = ?, machine_down_start =?, machine_down_finish=?, problem = ?, reason =? ,action_taken =? ,counter_messure =? ,cm_status =? ,completed_date =? ,charges = ? WHERE id = ?';
        return db.query(query, [report.createdDate, report.lineId, report.equipmentId, report.breakDownStart, report.breakDownFinish, report.machineDownStart, report.machineDownFinish, report.problem, report.reason, report.actionTaken, report.counterMessure, report.cmStatus, report.completedDate, report.charges, report.id], callback);
    },
};
